...
const Navbar = () => {
    ...
    return (
        <header className="app-header">
            <Link to="/tickets" style={{ display: 'flex', alignItems: 'center', gap: '1rem', textDecoration: 'none', color: 'white' }}>
            ...
            <nav className="flex items-center gap-8" style={{ marginLeft: '2rem' }}>
                {/* Staff / Admin / Technician Specific */}
                {(user?.role === 'ADMIN' || user?.role === 'TECHNICIAN' || user?.role === 'STAFF') && (
                    <Link to="/tickets" className="nav-link-custom">
                        Jobs & Tickets <Wrench size={18} />
                    </Link>
                )}
                
                {/* User Specific - Viewing their own tickets */}
                {user?.role === 'USER' && (
                    <Link to="/tickets" className="nav-link-custom">
            ...
            <div className="flex items-center gap-4 relative" style={{ marginLeft: 'auto' }}>
                ...
                {user ? (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginLeft: '0.5rem' }}>
                        <Link to="/tickets" style={{ color: 'rgba(255,255,255,0.9)', textDecoration: 'none', fontSize: '0.95rem', fontWeight: '500' }}>Guest</Link>
                    </div>
                )}
            </div>
        </header>
